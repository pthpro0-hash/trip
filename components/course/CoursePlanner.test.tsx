...
describe("CoursePlanner", () => {
  ...
  it("담은 곳이 없으면 무엇을 해야 하는지 알려준다", async () => {
    await renderWith([]);
    expect(screen.getByText("아직 담은 곳이 없어요")).toBeTruthy();
  ...
  it("위로 버튼을 누르면 순서가 바뀐다", async () => {
    ...
    expect(JSON.parse(window.localStorage.getItem(STORAGE_KEY)!)).toEqual(["경복궁", "창경궁"]);
  });
